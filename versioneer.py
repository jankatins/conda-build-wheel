...
def git_pieces_from_vcs(tag_prefix, root, verbose, run_command=run_command):
    ...
    if branch_name == 'HEAD':
        ...
        # the current commit. If all else fails, we are on a branchless
        # commit.
        branches = run_command(GITS,
                               ["branch", "--contains"],
                               cwd=root).split('\n')
        # Strip off the leading "* " from the list of branches.
        branches = [branch[2:]
                    for branch in branches if branch and branch[4:5] != '(']
        if 'master' in branches:
            branch_name = 'master'
        elif not branches:
            branch_name = None
...
def add_one_to_version(version_string, number_index_to_increment=-1):
    ...
    digit_parts = [(i, part) for i, part in enumerate(parts) if part.isdigit()]
...
def render_pep440_branch_based(pieces):
    ...
    # If we are on a tag, just pep440-pre it.
    if pieces["closest-tag"] and not (pieces["distance"] or pieces["dirty"]):
        rendered = pieces["closest-tag"]
    else:
        # Put a default closest-tag in.
        if not pieces["closest-tag"]:
